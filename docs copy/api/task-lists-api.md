...
Covers task lists, tasks, and task logs — the three collections behind [task-lists.md](../features/task-lists.md), [anytime-tasks.md](../features/anytime-tasks.md), and [timer.md](../features/timer.md). There used to be a separate API-key-authenticated variant of starting a timer for external triggers (Shortcuts, NFC silent triggers) — removed, see `docs/project-structure.md`'s note on the external API's removal; `nfc.md`'s Universal Link tap-to-trigger flow is the one external-trigger surface left, and it calls `lib/task-log-actions.ts` directly as a library function, not through a parallel authenticated route.
