...
**Manager UI**: the catalog is the bottom "Company Task Catalog" section of the **Manage Tasks** screen (`/tasks/manage`, manager-only, see `components/ManageTasksView.tsx`) — reached from a manager-only gear icon in the Tasks page header (`components/Header.tsx`'s `showManageLink`) or from the Profile page. It lists the full catalog regardless of which lists currently use each entry, is where a definition gets deleted (or fails to, with a message naming which lists still use it), and — since this split moved NFC binding onto the definition — is also where "Scan to Link"/"Unbind" happens directly (`POST`/`DELETE /api/task-definitions/[id]/nfc-tag`), working even for a definition not yet placed in any list. The same screen's top section lists every scheduled `TaskList` (tap through to `components/TaskListEditView.tsx` to rename/reschedule/delete it or manage its tasks, same screen as before) and a "+ Add Task List" button, always fully expanded (a small, bounded set); its middle section is a read-only overview of the anytime lists' tasks. From any list's edit page, "+ Use an existing task" (the "Your Saved Tasks" section inside `components/AddTaskSheet.tsx`'s browse view, wired through its `onAddExisting` prop — not a separate component) places an existing catalog entry into that list without creating a new definition — the original "+ Add task" flow (browsing `TaskTemplate`s or building custom) is unchanged and still always creates a brand-new definition.

**Compact rows, search, and Scan to Find**: Standalone Tasks and Company Task Catalog no longer render every item's field count/used-in/tag-binding/delete inline at once — each is a single-line compact row (icon, name, a one-line meta summary, chevron) that opens `components/ManageTaskDetailSheet.tsx` on tap, a bottom sheet (same presentation as `AddTaskSheet.tsx`) holding the "Used In" list, the NFC "Scan-to-Complete Tag" panel (catalog rows only — a Standalone Tasks row's sheet is lighter: name/icon/minutes plus Edit/Remove, no tag or used-in section, since a placement has neither concept), an "Edit in `<list>`" link, and a two-tap-confirm Delete/Remove. Both sections default collapsed once they pass `COLLAPSE_THRESHOLD` (5 items) — a tappable header shows the item count and a chevron; Task Lists is exempt, staying always-expanded per above. A sticky search box at the top of the screen filters all three sections by name at once, forcing any collapsed section open while a query is active. Collapse/search state is plain component state, not persisted across visits — CLAUDE.md rules out localStorage/sessionStorage for app state, and a MongoDB-backed per-user preference wasn't judged worth it for this. **Scan to Find** (next to the search box) is the companion for a physical tag whose raw UID a manager can't be expected to type: it scans and matches the catalog's own already-loaded `nfcTagUid` data client-side, then opens the matching definition's detail sheet directly (or a small picker if the tag backs more than one) — see [nfc.md](nfc.md#scan-to-find-manage-tasks) for the full flow.
...
The shared math lives in `lib/task-progress.ts`'s `computeWeeklyProgress` (imported by both `StreakDots` and the Reports Overview's Task Breakdown, see `reports.md`, so the two never diverge). Each of the week's 7 days classifies into one of six states — the first two are solid fills ("something happened"), the rest are hollow, no-fill outlines distinguished by border color/style ("needs a look"), deliberately so a close-but-different fill color is never the only thing telling two states apart:
...
- `app/(app)/tasks/manage/page.tsx`, `components/ManageTasksView.tsx` — the Manage Tasks screen: task lists, standalone tasks, and the company task catalog with NFC binding, search, and Scan to Find (see "Compact rows, search, and Scan to Find" above).
- `components/ManageTaskDetailSheet.tsx` — the shared detail-on-tap bottom sheet for a Standalone Tasks/Company Task Catalog compact row.
- `components/AddTaskSheet.tsx`'s "Your Saved Tasks" section (`onAddExisting`) — places an existing catalog entry into a list without creating a new definition.
